//! Module for magnetometers and combined peripherals.
...
/// Common trait for all magnetometers.
pub trait Magnetometer {
    type Error;
    type Output: Data;

    /// Returns the raw accelerometer data.
    fn magraw(&mut self) -> Result<[Self::Output; 3], Self::Error>;
    ...
    fn mag<F>(&mut self) -> Result<[F; 3], Self::Error>
        where F: Clone + Copy +
